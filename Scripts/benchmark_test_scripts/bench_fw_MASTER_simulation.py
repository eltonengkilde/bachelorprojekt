...
MAX_SIM_STEPS           = 1000
PER_HOP_CUTOFF          = 300   # stop benchmark if one hop exceeds this
...
print(f"Phase 1: out-deg max {int(out_deg.max())}, PageRank top hub: {node_list[max(range(g_full.num_vertices()), key=lambda i: pr_map[i])]}, SCCs: {hist.shape[0]}")
    ...
    try:
        with open(out_file, 'w') as fout, contextlib.redirect_stdout(fout):
            ...
            for i, genes in enumerate(sorted(decisions.values(), key=lambda x: x[0]), 1):
                print(f"  Decision {i}:")
                for g in sorted(genes): print(f"    {g:40s}{stag(g)}")
            ...
            for g in sorted(sink_nodes):
                ...
                print(f"    {g:40s}  resting: {r:8s}  perturbed: {p}")
            if community:
                print(f"\n{'='*70}")
                cg = {}
