...
while True:
    ...
    if target_gene not in node_idx:
        matches = [n for n in node_list if target_gene.upper() in n.upper()]
        print(f"WARNING: '{target_gene}' not found. Try again.")
        if matches: print(f"  Did you mean one of these? {matches[:5]}")
    else:
        print(f"OK: '{target_gene}' found in network"); break
...
for gene in direct_activators: print(f"    + {gene:40s}{stag(gene)}")
print(f"\n  Step 1 — Direct suppressors of '{target_gene}': {len(direct_suppressors)}")
for gene in direct_suppressors: print(f"    - {gene:40s}{stag(gene)}")

print(f"\n{'='*70}")
print(f"  Step 2 — Sufficient activators (activate alone from silent network): {len(sufficient_activators)}")
for gene in sufficient_activators: print(f"    + {gene:40s}{stag(gene)}")
...
for gene in necessary_activators: print(f"    ! {gene:40s}  [required]{stag(gene)}")
print(f"\n  Redundant activators ('{target_gene}' stays ON without them): {len(redundant_activators)}")
for gene in redundant_activators: print(f"    {gene:40s}  [dispensable]{stag(gene)}")
print(f"\n  Suppressors whose removal de-represses '{target_gene}': {len(suppressor_releases)}")
for gene in suppressor_releases: print(f"    ~ {gene:40s}  [KO turns target ON]{stag(gene)}")
...
if sink_nodes:
    ...
    for gene in sorted(sink_nodes):
        ...
        print(f"    {gene:40s}  permissive state: {'ON' if val else 'OFF'}")
