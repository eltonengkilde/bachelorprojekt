...
ACT_LABEL = "Activation / Induction / Causation / Result"
activations = {
...
SUP_LABEL = "Repression / Inhibition / Negative Regulation"
suppressions = {
...
rel_map = {**{k: ACT_LABEL for k in activations}, **{k: SUP_LABEL for k in suppressions}}

df = pd.read_csv(INPUT_FILE, sep=";", encoding="utf-8-sig")
total_rows = len(df)

df = df.apply(lambda c: c.str.strip() if c.dtype == object else c)
df = df[df["relationship"].isin(rel_map)].copy()
df["relationship_category"] = df["relationship"].map(rel_map)
df = df[["source", "source_category", "relationship_category", "target", "target_category"]]
