...
# Here the boolean logic is being made
...
print(f"\n Step 1: Direct activators of '{target_gene}' (from KG data): {len(direct_activators_of_target)} total")
for gene in direct_activators_of_target:
    print(f"    {gene}  [{gene_category.get(gene, '?')}]")

print(f"\n Step 1: Direct suppressors of '{target_gene}' (from KG data): {len(direct_suppressors_of_target)} total")
...
for gene in necessary_activators:
    print(f"    ! {gene}  [required]  [{gene_category.get(gene, '?')}]")
...
for gene in redundant_activators:
    print(f"     {gene}  [dispensable — another activator compensates]  [{gene_category.get(gene, '?')}]")

print(f"\n  Suppressors whose removal de-represses '{target_gene}': {len(suppressor_releases)} total")
for gene in suppressor_releases:
    print(f"     {gene}  [knocking this out turns target ON]  [{gene_category.get(gene, '?')}]")
